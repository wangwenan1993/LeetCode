...
public class _72_EditDistance {
    ...
    public static void main(String[] args) {
        System.out.println(dp("abcd", "bbcde"));
    }
     ...
     * @return
     */
    //还可优化空间复杂度为O(min(word1.length, word2.length))
        ...
        for(int i = 0; i < word1.length(); i++) {
            for(int j = 0; j < word2.length(); j++) {
                if(word1.charAt(i) == word2.charAt(j)) {
                    ...
                    int min_a = Math.min(step[i][j], step[i][j+1]);
                    int min_b = Math.min(min_a, step[i+1][j]);
                    step[i+1][j+1] = 1 + min_b;
                }
            }
        }
